...
When you run `muxed mcp --proxy-tools`, muxed starts an MCP server that registers a single `muxed` tool. This tool accepts a `command` string and an optional `input` object:
...
In Claude Desktop, the model sees a single `muxed` tool. When it needs to interact with your MCP servers, it calls this tool with commands like:

- `muxed("tools")` — discover what's available
- `muxed("grep database")` — find relevant tools
- `muxed("info postgres/query")` — check the schema
- `muxed("call postgres/query", { sql: "SELECT ..." })` — execute the call
