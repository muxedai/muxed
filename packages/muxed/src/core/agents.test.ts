...
import { mergeServers, writeMuxedConfig, modifyAgentConfig, getMuxedConfigPath } from './agents.js';

function makeTmpDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'muxed-agents-test-'));
}
...
describe('mergeServers', () => {
  it('imports servers into an empty muxed config', () => {
  ...
  it('skips servers that already exist in muxed config', () => {
...
describe('writeMuxedConfig', () => {
  ...
  it('creates a new config file with servers', () => {
    const configPath = path.join(tmpDir, 'muxed.config.json');
    const servers: Record<string, ServerConfig> = {
      filesystem: { command: 'npx', args: ['-y', 'fs-server'] },
    };

    writeMuxedConfig(configPath, servers);
  ...
  it('preserves existing daemon config when writing servers', () => {
    ...
    writeMuxedConfig(configPath, { github: { command: 'npx', args: ['github'] } });
  ...
  it('creates parent directories if needed', () => {
    ...
    writeMuxedConfig(configPath, { test: { command: 'echo' } });
...
describe('modifyAgentConfig', () => {
  ...
  it('removes servers and injects muxed entry when delete+replace', () => {
    const configPath = path.join(tmpDir, 'mcp.json');
    const original = {
  ...
  it('removes servers without injecting muxed when delete+no-replace', () => {
    const configPath = path.join(tmpDir, 'mcp.json');
...
describe('getMuxedConfigPath', () => {
  it('returns explicit path when provided', () => {
    expect(getMuxedConfigPath('local', '/my/custom/path.json')).toBe('/my/custom/path.json');
  });

  it('returns cwd-based path for local scope', () => {
    const result = getMuxedConfigPath('local');
    expect(result).toBe(path.join(process.cwd(), 'muxed.config.json'));
  });
