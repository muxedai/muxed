// Shared prompt template for CLI and MCP instruction modes.
// Both `muxed init` (file injection) and the MCP proxy import from here
// so the workflow documentation stays in sync.
...
export function makeCliFragments(run: string): Fragments {
  return {
    ...
    toolsSchema: (s) =>
      s ? `${run} muxed tools ${s} --include schema` : `${run} muxed tools --include schema`,
...
export function makeToolFragments(): Fragments {
  return {
    intro:
...
export function buildPrompt(f: Fragments, opts: PromptOptions = {}): string {
