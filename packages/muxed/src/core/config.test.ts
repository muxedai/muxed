...
function writeTmpConfig(dir: string, config: unknown): string {
  const filePath = path.join(dir, 'muxed.config.json');
...
describe('loadConfig', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'muxed-test-'));
  });
  ...
  it('merges Claude Desktop servers when mergeClaudeConfig is true', () => {
    ...
    // muxed servers always take precedence
    expect(config.mcpServers.overlap).toEqual({ command: 'muxed-overlap' });
  });
});
