...
import { getMuxedConfigPath } from './agents.js';
import type { MuxedConfig, ServerConfig } from './types.js';

export function getConfigPath(scope: 'local' | 'global', explicitPath?: string): string {
  return getMuxedConfigPath(scope, explicitPath);
}

export function readConfigFile(filePath: string): MuxedConfig {
...
export function writeConfigFile(filePath: string, config: MuxedConfig): void {
