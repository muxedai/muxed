...
import { discoverAgentConfigs, mergeServers, writeMuxedConfig } from '../../core/agents.js';
import type {
  ServerConfig,
  StdioServerConfig,
  HttpServerConfig,
...
export const mcpCommand = new Command('mcp')
  ...
  .option('--tools', 'Inject proxy MCP tool for clients without bash access')
  .action(async (opts: { tools?: boolean }, cmd: Command) => {
    ...
    await startMcpProxy({ configPath: explicitConfig, tools: opts.tools });
  });
...
mcpCommand
  ...
  .action(async (opts: { scope: string }) => {
    ...
    writeMuxedConfig(configPath, allServers);
    await tryReloadDaemon();
