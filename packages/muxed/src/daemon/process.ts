...
import { getMuxedDir, getPidPath, getSocketPath, ensureMuxedDir } from '../utils/paths.js';
...
function isMuxedProcess(pid: number): boolean {
  // Verify the PID is actually an muxed/node process
  try {
...
function acquireLock(): boolean {
  ...
  try {
    ...
    if ((err as NodeJS.ErrnoException).code === 'EEXIST') {
      // Lock file exists – check if the holder is still alive and is a muxed process
      try {
...
export async function cleanupStaleFiles(): Promise<void> {
  ...
  if (pid !== null && isProcessAlive(pid) && !isMuxedProcess(pid)) {
    // PID exists and alive, but not an muxed process – stale PID file
    try {
      fs.unlinkSync(pidPath);
    } catch {
      // ignore
