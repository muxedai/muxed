...
> Make `mcpd` consumable as a library from JavaScript/TypeScript scripts, not just the CLI.

## Motivation

The CLI is great for agents and shell scripts, but JS/TS users need a programmatic interface. They shouldn't have to shell out to `mcpd call` or speak JSON-RPC over a Unix socket manually. A proper API lets them:
...
5. **Separate entry point** — the API ships as `mcpd/client` (or `mcpd`), distinct from the CLI bundle. The CLI remains the `mcpd` bin; the library is an importable module.
...
// or with options:
const client = await createClient({ configPath: './mcpd.config.json' });
```
...
const tools = await mcpd.tools();
console.log(`Found ${tools.length} tools`);

const result = await mcpd.call('filesystem/read_file', {
  path: '/etc/hostname',
});
...
const matches = await mcpd.grep('search');
...
const handle = await mcpd.callAsync('server/long-running-tool', { input: 'data' });
console.log(`Task started: ${handle.taskId}`);

// Poll for completion
let status = await mcpd.task(handle.server, handle.taskId);
while (status.status !== 'completed' && status.status !== 'failed') {
  await new Promise((r) => setTimeout(r, 1000));
  status = await mcpd.task(handle.server, handle.taskId);
}

const result = await mcpd.taskResult(handle.server, handle.taskId);
```

### Error handling

```ts
import { createClient, McpdError } from 'mcpd';

const mcpd = await createClient();
try {
  await mcpd.call('server/nonexistent-tool');
} catch (err) {
